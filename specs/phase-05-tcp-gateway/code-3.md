...
**Depends on:** Phase 1 CLI complete.
...
## Task 5 — eventfd wakeup + engine-thread integration
**Depends on:** Task 4.
...
## Task 7 — ADR
**Depends on:** Task 5 (decision points settled by then).
- [ ] Prerequisite check: if Phase 1's pending ADR-003–005 are not yet
      written, write them as a sub-task first so `docs/adr/` has no
      gaps before Phase 5's ADR lands (may already be resolved when
      Phase 5 starts — check, don't assume).
- [ ] Write `ADR-NNN-client-id.md`, where NNN is the next available
      number in `docs/adr/` at implementation time: strong-typed
      `ClientId`, ephemeral per-connection, introduced in Phase 5 for
